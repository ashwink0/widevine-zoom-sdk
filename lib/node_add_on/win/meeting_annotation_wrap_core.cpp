...
ZNSDKError ZMeetingAnnotationWrap::StartAnnotation(ZNSDKViewType type, ZoomSTRING left, ZoomSTRING top)
{
	...
	return Map2WrapDefine(ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().StartAnnotation(viewtype));
}
...
bool ZMeetingAnnotationWrap::IsAnnotationLegalNoticeAvailable()
{
	return ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().IsAnnotationLegalNoticeAvailable();
}
ZoomSTRING ZMeetingAnnotationWrap::GetAnnotationLegalNoticesPrompt()
{
	ZoomSTRING zn_annoLegalNoticesPrompt;
	zn_annoLegalNoticesPrompt = ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().getAnnotationLegalNoticesPrompt();
	return zn_annoLegalNoticesPrompt;
}
ZoomSTRING ZMeetingAnnotationWrap::GetAnnotationLegalNoticesExplained()
{
	ZoomSTRING zn_annoLegalNoticesExpained;
	zn_annoLegalNoticesExpained = ZOOM_SDK_NAMESPACE::CSDKWrap::GetInst().GetMeetingServiceWrap().T_GetAnnotationController().getAnnotationLegalNoticesExplained();
	return zn_annoLegalNoticesExpained;
}